...
rdd = rdd.filter(lambda x:x!=header)\
         ...
         .map(lambda x: ",".join((x[0][1:-1]+"-"+x[1][1:-1],extract_value(x))))

#Saving intermediate file
rdd.saveAsTextFile("intermediate.csv")      
     
    
start = time.time()

#Reading intermediate file
data = sc.textFile("intermediate.csv")\
.map(lambda x:x.split(","))\
.map(lambda x:(x[0],x[1]))\
.groupByKey()\
.mapValues(set)\
.filter(lambda x:len(x[1])>filterk)\
.map(lambda x:x[1])\
.cache()
